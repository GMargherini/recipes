...
def recipes_page(base_url, lang, db):
    recipes = db.get_recipes_refs()
    for r in recipes:
        r["course"] = translation[lang][r["course"] if r["course"] else "entree"].title()
    with ui.row().classes('w-full'):
        ui.space()
        ui.button(icon='add', on_click= lambda: ui.navigate.to(f'/{base_url}/{lang}/recipe/new'))
    columns = [
    ...
    table = ui.aggrid({'columnDefs': columns, 'rowData': recipes, 'rowSelection': 'single'}, theme='quartz') \
        ...
        .on('cellClicked', lambda event: ui.navigate.to(f'/{base_url}/{lang}/recipes/{event.args['data']['id']}'))